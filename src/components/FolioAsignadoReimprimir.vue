<template>
  <div>
    <v-text-field
      ...
      dense
    ></v-text-field>
    <imprimir :opcion="'todos'" :cupon="folioAsignadosReimprimir" :id="'0'" v-show="folioAsignadosReimprimir.length!=0" />
    <v-row >
      <v-col cols="6" class="pa-2" v-for="(folio,i) in folioAsignadosReimprimir" :key="folio.id">
        <v-card class="card" >
          ...
          <div>
            <div class="d-flex flex-row justify-center align-center">
              <button class="estudioInfo" @click="folioAsignadosReimprimir[i].flecha=true">
              ...
              <button class="estudioInfo bl-1-gray" @click="folioAsignadosReimprimir[i].flecha=false">
...
export default {
  ...
  data: () => ({
    ...
    filtroSearch:null,
    loading:false
    // folioAsignadosReimprimir:[{estudioId:2,estudio:'Laboratorio',nombre:'Christian', apellidoP:'Pulido',apellidoM:'Quintero', clinica:'Navolato',status:0,folio:14021996, preparacion:'Sin preparacion'}]
  }),
  mounted(){
  ...
  methods:{ 
   getFoliosAsignados(filtro){
     console.log(filtro)
      foliosService.getAsignadosImpresos(this.dataUser.institution.id,filtro).then(res=>{
        let data=res.map(x=>{
        x.flecha=true
        return x
        })
        this.folioAsignadosReimprimir=data
        this.loading=false
      })
    },
    llenarDatos(f){
